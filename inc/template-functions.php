 ...
 * @return array
 */
function happyprime_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'happyprime_filter_script_loader_tag', 10, 2 );
 ...
 */
function happyprime_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function happyprime_add_body_style() {
	...
	// Preload content.css.
	$preloads['happyprime-content'] = happyprime_get_preload_stylesheet_uri( $wp_styles, 'happyprime-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['happyprime-sidebar'] = happyprime_get_preload_stylesheet_uri( $wp_styles, 'happyprime-sidebar' );
		$preloads['happyprime-widgets'] = happyprime_get_preload_stylesheet_uri( $wp_styles, 'happyprime-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['happyprime-comments'] = happyprime_get_preload_stylesheet_uri( $wp_styles, 'happyprime-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['happyprime-front-page'] = happyprime_get_preload_stylesheet_uri( $wp_styles, 'happyprime-front-page' );
	}
...
add_action( 'wp_head', 'happyprime_add_body_style' );
 ...
 */
function happyprime_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'happyprime_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function happyprime_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'happyprime_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'happyprime_add_nav_menu_aria_current', 10, 2 );
